...
namespace INCHEQS.ConfigVerificationBranch.BranchActivation
{
    public interface IBranchActivationDao {
        ...
        List<BranchActivationModel> GetCutOffTimes(string clearDate);

        BranchActivationModel GetChequeActivation(string clearDate);
        DataTable GetChequesActivation(string clearDate);

        void InsertCutOffTime(string activation, string cutOffTimeFrom, string cutOffTime, String currentUser,string clearingType);
